...
<style>
  .ur-table .warning-row {
    background: oldlace;
  }

  .ur-table .success-row {
    background: #f0f9eb;
  }
</style>
...
只需要在 ur-table-column 里面嵌套 ur-table-column，就可以实现多级表头。
```html
<template>
  <ur-table
    :data="tableData"
    ...
    <ur-table-column label="配送信息">
      <ur-table-column
        prop="name"
        label="姓名"
        width="120">
      </ur-table-column>
      <ur-table-column label="地址">
        <ur-table-column
          prop="province"
          label="省份"
          width="120">
...
Table 组件提供了单选的支持，只需要配置`highlight-current-row`属性即可实现单选。之后由`current-change`事件来管理选中时触发的事件，它会传入`currentRow`，`oldCurrentRow`。如果需要显示索引，可以增加一列`ur-table-column`，设置`type`属性为`index`即可显示从 1 开始的索引号。
```html
<template>
  <ur-table
    ref="singleTable"
  ...
  <div style="margin-top: 20px">
    <ur-button @click="setCurrent(tableData[1])">选中第二行</ur-button>
    <ur-button @click="setCurrent()">取消选择</ur-button>
  </div>
</template>
...
通过设置 [Scoped slot](https://cn.vuejs.org/v2/guide/components-slots.html#%E4%BD%9C%E7%94%A8%E5%9F%9F%E6%8F%92%E6%A7%BD) 来自定义表头。
```html
<template>
  <ur-table
    ...
    <ur-table-column
      ...
      <template #default="scope">
        <ur-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Edit</ur-button>
        <ur-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Delete</ur-button>
      </template>
    </ur-table-column>
  </ur-table>
</template>
